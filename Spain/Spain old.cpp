...
void Script(){
	...
	{ //500		
		// Krepost
		city->PKrepost=30;
		if(!(UpgIsRun(&Spn.Upg.Mel_GETRES2)||GetMoney(FOOD)>12000)||NU<40) city->PKrepost=NU>>4;
		if(city->PKrepost>30) city->PKrepost=30;
		// Field
	...
	if(city->MinWooders>120) TryUnit(&Spn.Build.Skl, 2, 100, 95);

	// Units in buildings
	city->NStrelok = 10;
	if(GetUnits(&Spn.Build.Do2)) city->NStrelok = 20;
	city->NUnits = 2;
	...
	int S17Perc=100;
	if(GetUnits(&Spn.Build.Kr8)) S17Perc=50;

	TryUnit(&Spn.Unit.S15, 10000, 20, 50);
	TryUnit(&Spn.Unit.S15, 10000, 20, 50);
	TryUnit(&Spn.Unit.S15, 10000, 20, 50);
	...
	TryUnit(&Spn.Unit.S18, 200, 20, 100);
	TryUnit(&Spn.Unit.S18, 200, 20, 100);
	TryUnit(&Spn.Unit.S18, 200, 20, 100);
	...
	// Buildings
	{
		...
		if(UpgIsRun(&Spn.Upg.Mel_GETRES)){
	...
	// Blg
	{
		if(GetReadyUnits(&Spn.Build.Do1)>3){
			TryUnit(&Spn.Build.Blg, 2, 100, 70);
		}
		if(GetReadyUnits(&Spn.Build.Kr8)){
			TryUnit(&Spn.Build.Blg, 3, 100, 70);
		}
		if(GetReadyUnits(&Spn.Build.Cen)){
			TryUnit(&Spn.Build.Blg2, 3, 100, 70);
		}
	}
	...
	if(UpgIsRun(&Spn.Upg.Mel_GETRES2)) TryUpgrade(&Spn.Upg.CEN55, 100, 100);
	...
	// Mine
	{
		...
		if(GetDifficulty()>1 && UpgIsRun(&Spn.Upg.Mel_GETRES2)){
			...
			TryUpgradeEx(&Spn.Upg.Stn1, 90, 80, 6000, 1000);
			TryUpgradeEx(&Spn.Upg.Stn2, 90, 80, 6000, 1000);
			...
			//}
			TryUpgradeEx(&Spn.Upg.Iro0, 80, 100, 6000, 1000);
			TryUpgradeEx(&Spn.Upg.Iro1, 70, 100, 6000, 1000);
			TryUpgradeEx(&Spn.Upg.Iro2, 60, 100, 6000, 1000);
			TryUpgradeEx(&Spn.Upg.Iro3, 60, 100, 6000, 1000);
	...
	if(GetUnits(&Spn.Build.Do2)){
		...
		TryUpgrade(&Spn.Upg.Kr8_Stage1, 90, 100);
	}
...
void WarriorsUpg(){
	...
	// Cen
	{
		...
		TryUpgrade(&Spn.Upg.ArmShield2,50,80);
	...
	if(UpgIsRun(&Spn.Upg.Mel_GETRES2)){
		if(UpgIsRun(&Spn.Upg.S15Protect7)&&GetUnits(&Spn.Unit.S15)>60){
			TryUpgrade(&Spn.Upg.S15Attack, 10, 100);
			TryUpgrade(&Spn.Upg.S15Attack3, 10, 100);
			TryUpgrade(&Spn.Upg.S15Attack4, 10, 100);
			TryUpgrade(&Spn.Upg.S15Attack5, 10, 100);
			TryUpgrade(&Spn.Upg.S15Attack6, 10, 100);
			TryUpgrade(&Spn.Upg.S15Attack7, 5, 100);
		}
		...
		if(GetUnits(&Spn.Unit.S15)>60){
			...
			TryUpgrade(&Spn.Upg.S15Protect6, 40, 100);
			TryUpgrade(&Spn.Upg.S15Protect7, 40, 100);
		}
	}
